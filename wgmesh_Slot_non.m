function [x, y, xc, yc, nx, ny, eps] = wgmesh_Slot_non(n,h,rh,rw,side,rw_s_half,n_s,dx,dy);
...
% imagesc(eps');
 ...
 iy1   = sum(ih)-ih(nlayers)-irh+1;
 
 delta = (irh - rh/dy)*dy;
 
 y(iy1) = y(iy1) + delta;
 
 yc(iy1) = (y(iy1)+y(iy1+1))/2;
 
 yc(iy1-1) = (y(iy1)+y(iy1-1))/2;
 ...
 iy2   = sum(ih)-ih(nlayers)+irh_s-irh+1;
 
 delta = (irh - rh/dy)*dy;
 
 y(iy2) = y(iy2) + delta;
 
 yc(iy2) = (y(iy2)+y(iy1+2))/2;
 
 yc(iy2-1) = (y(iy2)+y(iy2-1))/2; 
